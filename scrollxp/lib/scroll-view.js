...
class ScrollView {
  constructor (options) {
    ...
    new BreakpointListener(({ screenSize, hasChanged }) => {
      if (hasChanged) {
        if (!this._controller) { // First load
        ...
        } else if (this._smoothScrolling) { // Disable smooth scrolling on mobile
          if (screenSize === 'xs') {
  ...
  _createAnimation (scene, domScene) {
    ...
    domElements.forEach((domElement) => {
      ...
      if (animation) { // Retrieve registered animation
      ...
      } else { // Build animation from DOM attributes
  ...
  _buildParallaxScenes () {
    ...
    domElements.forEach((domElement) => {
      ...
      if (isEnabled) {
        ...
        if (parallaxType === 'global') { // Global items
          globalItems.push(item)
        } else if (parallaxType === 'scene') { // Scene items
